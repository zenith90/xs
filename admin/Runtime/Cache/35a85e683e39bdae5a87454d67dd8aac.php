		...
		<div id="left_layout">
			...
			<div id="main_content" class="container-fluid">
					...
					<div class="span6">
						...
						<div class="well widget">
							...
							<div class="widget-header">
								<h3 class="title"><B><?php echo ($novelinfo["novelname"]); ?></B>内容列表</h3>
							</div>
							<!-- ./ widget header -->
							...
							<div class="widget-content">
								<table class="table table-hover">
									<thead>
										<tr>
											<th><input type="checkbox" value="" class="fancy"  id="selectAll" onClick="selectAllCheckBox('product',this.checked)"/>内容 ID</th>
											<th>内容标题</th>
											<th>所属分卷</th>
											<th>操作</th>
										</tr>
									...
									<?php if(is_array($contents)): $i = 0; $__LIST__ = $contents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$content): $mod = ($i % 2 );++$i;?><tr>
											<td><input type="checkbox"id="<?php echo ($content["id"]); ?>" class="fancy" /><?php echo ($content["id"]); ?></td>
											<td><?php echo ($content["con_name"]); ?></td>
											<td><?php echo ($content["volname"]); ?></td>
											<td> <a class="btn btn-small  btn-primary" href="__URL__/edit/novelid/<?php echo ($novelinfo["id"]); ?>/id/<?php echo ($content["id"]); ?>">修改</a>  
											<a class="btn btn-mini  btn-primary" href="__URL__/del/novelid/<?php echo ($novelinfo["id"]); ?>/id/<?php echo ($content["id"]); ?>" onclick="return confirm('确定要删除章节<?php echo ($content["con_name"]); ?>吗？')">删除</a> </td>
				...
				<div class="btn-toolbar2">
						<a class="btn btn-inverse btn btn-large btn-primary"  onClick="delMany('product')">删除所选内容</a>
		...
		function delMany(ParentID){
			if(confirm('确定要删除所选章节吗？')){
				...
				for(var i=0;i<span.length;i++){
					...
					window.location="__APP__/Chapter/delMany/novelid/<?php echo ($novelinfo["id"]); ?>/id/"+get; 
				}
			}
